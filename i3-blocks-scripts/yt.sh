...
function usage () {
  echo "usage: yt"
  echo "    -h        help"
  echo "    -C        choose custom fps and resolution"
...
#   case "${arg}" in
#     e)
# max_resolution="$(echo "480
# 720
# 360
# 1080
# 144" | dmenu -p resolution -i -l 5)"
# max_fps="$(echo "30
# 60" | dmenu -p fps -i -l 2)" 
#       ;;
#   esac
# done
...
if [ -z "$query" ]; then
  ...
  if [[ $action = "c" ]]; then
    ...
    # if in gui mode use gui prompt
    if [[ $isGui = "t" ]]; then
      query=$(echo | $promptcmd -p "Search $prompt_search: ")
      promptcmd="$promptcmd -p $prompt_search:"
    else
      echo -n "Search: "
      read -r query
    fi
