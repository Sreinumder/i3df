return function(INPUT_LINE_NUMBER)
    ...
    vim.api.nvim_buf_set_keymap(term_buf, 'x', 'H', '^', { })
    vim.api.nvim_buf_set_keymap(term_buf, 'x', 'L', 'g_', { })
    ...
    local setCursor = function()
      ...
      vim.cmd [[
          if search('❯ \n', 'w')
              " Cursor jumps automatically to match
              "return
              normal! 2l
            else
              " If not found, go to middle of screen
              normal! M
            endif
        ]]
    end
