...
map({ "n", "v", "o" }, "H", "^", { desc = "Start of line" }) map("n", "<A-H>", "H", { desc = "Default H" })
map({ "n", "v", "o" }, "M", "gM", { desc = "Middle of Line" }) map("n", "<A-M>", "M", { desc = "Default M" })
...
map("n", "<C-u>", "<C-u>zz", { desc = "Half page up" }) map("n", "<C-d>", "<C-d>zz", { desc = "Half page down" })
map("x", "<", "<gv") map("x", ">", ">gv") -- Continuous visual shifting (does not exit Visual mode), `gv` means
...
map({ "n", "v", "x" }, "x", '"_x') map({ "n", "v", "x" }, "X", '"_X')
map({ "n", "v", "x" }, "d", '"_d') map({ "n", "v", "x" }, "D", '"_D')
map({ "n", "v", "x" }, "c", '"_c') map({ "n", "v", "x" }, "C", '"_C')
map({ "n", "v", "x" }, "<A-x>", "x") map({ "n", "v", "x" }, "<A-X>", "X")
map({ "n", "v", "x" }, "<A-d>", "d") map({ "n", "v", "x" }, "<A-D>", "D")
map({ "n", "v", "x" }, "<A-c>", "c") map({ "n", "v", "x" }, "<A-C>", "C")
map({ "n", "v", "x" }, "<A-d><A-d>", "dd") map({ "n", "v", "x" }, "<A-c><A-c>", "cc")
map({ "v", "x" }, "p", '"_dP') map({ "v", "x" }, "P", '"_dp')
map({ "v", "x" }, "<A-p>", "p") map({ "v", "x" }, "<A-P>", "P")
...
map({ "i", "c" }, "<C-A>", "<HOME>") map({ "i", "c" }, "<C-E>", "<END>")
...
map("n", "<A-o>", 'i<enter><enter><Up><esc>!! | sed -z "s/\\n/, /g"<Home><Right><Right>', {desc = "genearate comma seperated seq or shuf or any bash output"})
map("i", "<A-o>", '<enter><enter><Up><esc>!! | sed -z "s/\\n/, /g"<Home><Right><Right>', {desc = "genearate comma seperated seq or shuf or any bash output"})
map("x", "<A-o>", '<esc>a<Enter><esc>gvo<esc>i<Enter><esc>^"_d0vg_!sed "s/, */\\n/g"| grep -v "^$" |  | sed -z "s/\\n/, /g"<C-Left><C-Left><C-Left><C-Left><C-Left><C-Left>')
map("x", "<leader>oso", '<esc>a<Enter><esc>gvo<esc>i<Enter><esc>^"_d0vg_!sed "s/, */\\n/g"| grep -v "^$" | sort -g | sed -z "s/\\n/, /g"<CR>kJxJhx')
map("x", "<leader>osr", '<esc>a<Enter><esc>gvo<esc>i<Enter><esc>^"_d0vg_!sed "s/, */\\n/g"| grep -v "^$" | sort -gr | sed -z "s/\\n/, /g"<CR>kJxJhx')
map("x", "<leader>oss", '<esc>a<Enter><esc>gvo<esc>i<Enter><esc>^"_d0vg_!sed "s/, */\\n/g"| grep -v "^$" | shuf | sed -z "s/\\n/, /g"<CR>kJxJhx')
map("x", "<leader>on", '<esc>a<Enter><esc>gvo<esc>i<Enter><esc>^"_d0vg_!sed "s/, */\\n/g"| grep -v "^$" | wc -l<CR>kJxJhx')
